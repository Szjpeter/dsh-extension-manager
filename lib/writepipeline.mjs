...
/**
 * Collect row ids from every composition layer we can see. The file being
 * written IS included in the raw scan; `previewCompositionWrite` filters out
 * rows sourced from that exact path, because superseding our own previous
 * generation is a legal rewrite, not a collision.
 *
...
export function previewCompositionWrite({ filePath, nextText, profileDir, dshHome }) {
  ...
  // Internal uniqueness applies WITHIN insert ids only. An insert plus a
  // same-id override row (the `{id, disabled}` toggle vocabulary) is the
  // LEGAL layering every managed document relies on — flagging it would
  // break every toggle write. Duplicated overrides are safe by design
  // (last-wins merge).
  const seen = new Set()
  ...
  if (profileDir && dshHome) {
    ...
    // scanLayerIds walks EVERY visible layer including the file being written;
    // rows contributed by this file's PREVIOUS generation must not count as a
    // "foreign layer" collision — rewriting our own rows is the normal flow.
    const here = path.resolve(filePath)
    const foreign = (source) => {
      try {
        return source ? path.resolve(source) !== here : true
      } catch {
        return true
      }
    }
    for (const e of layers.errors) problems.push(`其他配置层解析失败（拒绝在未知状态下写入）：${e}`)
    // Only ids the NEW document actually inserts can collide.
    for (const id of inserts) {
      const source = layers.inserts.get(id)
      if (!source) continue
      if (!foreign(source)) continue
      problems.push(
        `行 id「${id}」已在其他层注册为 insert（${source}）——重复 insert 会导致 dsh web 无法启动，已拒绝写入`
      )
    }
  }
