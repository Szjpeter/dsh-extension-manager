...
export function parseYaml(text) {
  // Strip a leading UTF-8 BOM: PowerShell-edited files (Out-File/Set-Content)
  // carry EF BB BF, and without this the BOM glues onto the first "- id"
  // line, turning whole SEQUENCES into bogus one-key mappings (the 08-27
  // incident that blanked the MCP/Skills pages). Immunity at the parser is
  // the single choke point every composition/frontmatter read funnels through.
  ...
  function parseBlockScalar(lineIdx, header) {
    ...
    let p = lineIdx + 1
    // Only BLANK lines may be skipped while detecting the content indent.
    // Comment-looking lines are CONTENT inside a block scalar ("# heading" is
    // common as the first line of a skill description); skipping them here
    // silently dropped those leading lines from the parsed value (and a later
    // manager write persisted the loss).
    while (p < lines.length) {
      if (isBlank(lines[p])) { p += 1; continue }
...
export function splitFrontmatter(text) {
  // BOM strip is load-bearing here: this is RAW TEXT surgery (no parseYaml
  // funnel), and a BOM'd SKILL.md (PowerShell-edited) would otherwise fail
  // the startsWith('---') check — reads masked every field, and a later
  // updateSkill rewrote the old frontmatter into the body (data corruption).
