...
/**
 * Search GitHub for repositories whose NAME matches `name` (used to
 * auto-resolve the update source for ZIP/manual plugin installs). Returns
 * candidate full_names, exact repo-name matches first.
 */
export async function searchRepositoriesByName(name) {
  const clean = encodeURIComponent(String(name || '').trim())
  if (!clean) return []
  const url = `${API}/search/repositories?q=${clean}+in:name&per_page=10`
  let res
  try {
    res = await fetch(url, { headers: ghHeaders(), signal: AbortSignal.timeout(15000) })
  } catch (error) {
    throw new Error(`网络请求失败：${error && error.message ? error.message : String(error)}`)
  }
  if (res.status === 403) throw new Error('GitHub 搜索配额受限，请稍后再试')
  if (!res.ok) throw new Error(`GitHub HTTP ${res.status}`)
  const data = await res.json()
  const items = Array.isArray(data.items) ? data.items : []
  const lower = String(name).toLowerCase()
  const exact = items.filter((r) => String(r.name || '').toLowerCase() === lower)
  const rest = items.filter((r) => String(r.name || '').toLowerCase() !== lower)
  return [...exact, ...rest].map((r) => r.full_name)
}
