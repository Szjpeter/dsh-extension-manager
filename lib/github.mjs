 ...
 */
export async function detectRepoUnits(repo, ref) {
  return detectRepoUnitsWithLister((dir) => listContents(repo, ref, dir))
}

/** Same walk, but the directory-listing source is injectable (MCP bridge). */
export async function detectRepoUnitsWithLister(listDir, readFile) {
  ...
  async function walk(dir, depth) {
    ...
    if (hasPkg) {
      const cls = await classifyPackage(dir)
      plugins.push({
        path: dir || '',
        label: dir || '(仓库根目录)',
        kind: cls.kind,
...
//   dsh-plugin  → package.json carries the harness `dsh` field
//   mcp-server  → MCP 服务包（bin 入口或名称含 mcp）：以服务器形式运行，不走插件安装
//   unknown     → 其余 npm 包：与 DSH 插件机制无关
async function classifyPackage(dir) {
  if (!readFileFn) return { kind: 'unknown' }
  try {
 ...
 * a confirmation in that case. Write goes through the same atomic writer the
 * rest of the plugin uses (temp + rename).
 */
export async function installSkillFromRepo({ repo, ref, filePath, suggestedName, force }) {
  ...
  const name = kebab(suggestedName || deriveSkillName(filePath) || ('gh-' + Date.now()))
  if (!name) throw new Error('无法从路径推导技能名，请提供名称')
  const dir = path.join(dshHome(), 'skills', name)
  const file = path.join(dir, 'SKILL.md')
  const exists = fs.existsSync(file)
  if (exists && !force) {
    return { ok: false, exists: true, name, file }
  }
  fs.mkdirSync(dir, { recursive: true })
  const tmp = file + '.tmp'
  fs.writeFileSync(tmp, text, 'utf8')
  fs.renameSync(tmp, file)
  return { ok: true, name, file, overwritten: exists }
}
