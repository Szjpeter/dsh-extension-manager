...
// Single source of truth for the slug normalizer (also consumed by
// github.mjs / list.mjs); re-exported so existing imports keep working.
export { kebab }
...
export function createSkill(input, scope, cwd = process.cwd()) {
  ...
  writeFileAtomic(file, renderSkillMd({ ...input, name }))
...
export function updateSkill(name, input, scope, cwd = process.cwd()) {
  ...
  if (nextName === name) {
    writeFileAtomic(current.file, content)
