 ...
 * ONE button, backend decides — npm first, release fallback, clone fallback,
 * else 无法安装).
 *   npm    : package published on npm under the repo's base name
 *   release: repo ships update/stable.json (author-maintained version file)
 *            AND a matching release asset exists on GitHub Releases
 *   clone  : not on npm/release, but the repo's package.json passes clone
 *            rules (main entry committed + zero runtime deps)
...
export async function chooseInstallChannel(repo, subdir = '') {
  ...
  try {
    ...
    // registry unreachable — fall through to release probing
  }
  // 2) GitHub Release channel probe: the repo maintains update/stable.json
  //    and a release asset named `<pkgName>-<version>.tgz` exists. Only
  //    relevant when a (sub)dir package.json provides the package name.
  try {
    const pkgPath = (subdir ? subdir + '/' : '') + 'package.json'
    const pkg = JSON.parse(await fetchRawFile(repo, 'HEAD', pkgPath))
    if (pkg && typeof pkg.name === 'string' && pkg.name !== '') {
      const info = await resolveReleaseInfo(repo, 'update/stable.json')
      const assetName = releaseAssetName(`${pkg.name}-<version>.tgz`, info.version)
      // Release downloads answer with a 3xx redirect to the CDN; `redirect:
      // manual` keeps the probe cheap (no body download). Any mirror hit
      // qualifies the repo as release-distributed.
      let releaseProbeHit = false
      for (const { base, tag } of info.bases) {
        try {
          const probe = await fetch(`${base}/${encodeURIComponent(tag)}/${encodeURIComponent(assetName)}`, {
            method: 'HEAD',
            redirect: 'manual',
            signal: AbortSignal.timeout(15000),
          })
          if (probe.status < 400) { releaseProbeHit = true; break }
        } catch {
          // try next mirror
        }
      }
      if (releaseProbeHit) {
        return { channel: 'release', release: { assetPattern: `${pkg.name}-<version>.tgz`, versionFile: 'update/stable.json' } }
      }
    }
  } catch {
    // no version file / no package.json / asset missing — fall through
  }
  // 3) clone preflight: read the remote package.json (raw 404 also proves a
...
/**
 * ONE install entry for the Git tab (user ruling): npm first, release
 * fallback, clone fallback, otherwise a final 无法安装 answer. Messages
 * record which channel served.
 */
export async function installPluginAuto(webProfileDir, repo, subdir) {
  ...
  if (choice.channel === 'release') {
    const r = await installReleasePlugin(webProfileDir, repo, choice.release)
    return { ...r, channel: 'release' }
  }
...
/**
 * Install a plugin from a GitHub Release tgz (npm-pack layout). The asset is
 * the COMPLETE built plugin package — no build step needed — so it is
 * self-contained regardless of any repo subdir. Installs into the managed
 * plugins dir, registers a file:// row, records extended provenance, and
 * links peerDependencies (e.g. @deepseek-ai/dsh-mcp-client) so bare imports
 * resolve outside the profile/runtime trees.
 */
export async function installReleasePlugin(webProfileDir, repo, release) {
  ...
  if (fs.existsSync(target)) {
    const verdict = validateClonePackage(target)
    if (verdict.ok) {
      const reg = registerCloneRow(webProfileDir, slug, target, verdict.pkg)
      if (!reg.ok) {
        return { ok: false, code: 'invalid', message: `目录 ${slug} 已存在且校验通过，但${reg.message}` }
      }
      return { ok: true, message: `检测到 ${full} 已安装但未注册，已补写注册行，重启 dsh web 后生效`, path: target }
    }
    fs.rmSync(target, { recursive: true, force: true })
  }

  const releaseCfg = release && typeof release === 'object' && typeof release.assetPattern === 'string'
    ? release
    : { assetPattern: `${slug}-<version>.tgz`, versionFile: 'update/stable.json' }
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-rel-'))
  try {
    const info = await resolveReleaseInfo(full, releaseCfg.versionFile || 'update/stable.json', releaseCfg.mirrors || [])
    const assetName = releaseAssetName(releaseCfg.assetPattern, info.version)
    const tgzPath = path.join(tmpDir, 'pkg.tgz')
    let fetched
    try {
      fetched = await fetchReleaseAsset(info, assetName)
    } catch (error) {
      return { ok: false, code: 'network', message: `下载发行包失败（${assetName}）：${error && error.message ? error.message : String(error)}` }
    }
    fs.writeFileSync(tgzPath, Buffer.from(await fetched.res.arrayBuffer()))
    const srcBase = await extractTgzRoot(tgzPath, tmpDir)
    const verdict = validateClonePackage(srcBase)
    if (!verdict.ok) return { ok: false, code: 'invalid', message: '发行包未通过安装校验（' + verdict.message + '）' }
    const pkgName = verdict.pkg && typeof verdict.pkg.name === 'string' ? verdict.pkg.name : ''
    const pkgVersion = verdict.pkg && typeof verdict.pkg.version === 'string' ? verdict.pkg.version.replace(/^v/, '') : ''
    if (pkgVersion !== info.version) {
      return { ok: false, code: 'invalid', message: `发行包版本不符（包内 ${pkgVersion || '?'} ≠ 版本文件 ${info.version}），已取消` }
    }
    fs.cpSync(srcBase, target, { recursive: true })
    const reg = registerCloneRow(webProfileDir, slug, target, verdict.pkg)
    if (!reg.ok) {
      fs.rmSync(target, { recursive: true, force: true })
      return { ok: false, code: 'invalid', message: '注册行未能写入配置：' + reg.message }
    }
    writePluginOrigin(target, full, releaseCfg)
    rememberReleaseMirrors(target, info.meta)
    const link = linkPeerDeps(target, verdict.pkg, releaseCfg.linkSource)
    rememberLinkSource(target, link.source)
    let message = `已通过 GitHub Release 安装 ${pkgName} v${pkgVersion}（${full}，下载源：${fetched.host}），重启 dsh web 后生效`
    if (link.linked && link.linked.length > 0) message += '；已链接对等依赖：' + link.linked.join(', ')
    if (link.skipped && link.skipped.length > 0) message += '；⚠ 对等依赖未链接：' + link.skipped.join(', ')
...
export async function installNpmPlugin(webProfileDir, pkgName, expectedRepo) {
  ...
  try {
    ...
    // npm-channel installs bypass npm's own dependency resolution: runtime
    // deps declared by the plugin must be fetched explicitly, one level deep
    // (deps of deps remain the profile's pnpm concern). Failures degrade to
    // the old warning instead of failing the install.
    const depRes = await resolveRuntimeDeps(webProfileDir, pkgJson.dependencies, pp ? pp.dependencies : null)
    if (depRes.installed.length > 0 && pp && pp.dependencies) {
      try { fs.writeFileSync(profilePkgPath, JSON.stringify(pp, null, 2) + '\n', 'utf8') } catch { /* best-effort */ }
    }
    const bundleWarn = bundlePatchWarning(pkgJson)
    let message = `已安装 ${name}@${version}（npm），重启 dsh web 后生效`
    if (depRes.installed.length > 0) {
      message += `；已自动安装依赖 ${depRes.installed.join('、')}`
    }
    if (depRes.failed.length > 0) {
      message += `；注意：依赖自动安装失败（${depRes.failed.join('、')}），重启后若插件加载失败请手动安装`
    }
    if (bundleWarn !== '') {
      message += `；${bundleWarn}`
    }
...
export function listPlugins(ctx, webProfileDir) {
  ...
  if (loader) {
    ...
    for (const entry of loader.entries()) {
      ...
      const entryId = String(entry.id).replace(/^include:/, '')
      // file:// rows surface their on-disk path as `name` — the UI should show
      // the entry id (the repo/plugin name) instead of a raw filesystem path.
      const displayName = path.isAbsolute(String(name)) ? entryId : name
      plugins.push({
        entryId: entry.id,
        name,
        displayName,
 ...
 * installs, GitHub Release tgz installs): a JSON the installer (human or AI)
 * drops next to package.json. Without it a source-less dir cannot be
 * update-checked at all.
 *   { "repo": "owner/repo" }
 *   { "repo": "owner/repo", "release": { "assetPattern": "pkg-<version>.tgz",
 *     "versionFile": "update/stable.json" } }   ← GitHub Release channel
 * `assetPattern` uses a literal `<version>` placeholder; `versionFile`
 * defaults to `update/stable.json` and is read from the repo's raw HEAD.
 */
export function writePluginOrigin(dir, repo, release) {
  const clean = String(repo || '').trim()
  if (!/^([\w.-]+)\/([\w.-]+)$/.test(clean)) return false
  const origin = { repo: clean, installedAt: new Date().toISOString() }
  if (release && typeof release === 'object') {
    if (typeof release.assetPattern === 'string' && release.assetPattern !== '') {
      origin.release = {
        assetPattern: release.assetPattern,
        versionFile: typeof release.versionFile === 'string' && release.versionFile !== ''
          ? release.versionFile
          : 'update/stable.json',
      }
      if (Array.isArray(release.mirrors)) {
        const mirrors = release.mirrors.filter((m) => typeof m === 'string' && m.includes('/'))
        if (mirrors.length > 0) origin.release.mirrors = mirrors
      }
      if (typeof release.linkSource === 'string' && release.linkSource !== '') {
        origin.release.linkSource = release.linkSource
      }
    }
  }
  try {
...
/** Full provenance object: { repo, release? } or null when unreadable. */
function readPluginOriginFull(dir) {
  try {
    const v = JSON.parse(fs.readFileSync(path.join(dir, '.dsh-plugin-origin.json'), 'utf8'))
    if (!v || typeof v.repo !== 'string' || !v.repo.includes('/')) return null
    const out = { repo: v.repo }
    if (v.release && typeof v.release === 'object' && typeof v.release.assetPattern === 'string' && v.release.assetPattern !== '') {
      out.release = {
        assetPattern: v.release.assetPattern,
        versionFile: typeof v.release.versionFile === 'string' && v.release.versionFile !== ''
          ? v.release.versionFile
          : 'update/stable.json',
      }
      if (Array.isArray(v.release.mirrors)) {
        out.release.mirrors = v.release.mirrors.filter((m) => typeof m === 'string' && m.includes('/'))
      }
      if (typeof v.release.linkSource === 'string' && v.release.linkSource !== '') {
        out.release.linkSource = v.release.linkSource
      }
    }
    return out
  } catch {
...
// ── GitHub Release channel (release tgz installs) ──────────────────────────

/**
 * Read the author-maintained version file from the repo's raw HEAD
 * (e.g. update/stable.json: { "latestVersion": "0.2.8", "releaseUrls": {...} }).
 * Tries GitHub raw first, then Gitee raw mirrors (main/master branches) —
 * the version file often stays reachable on a mirror when GitHub's raw/CDN
 * endpoints are blocked or flaky. Returns the latest version and an ordered
 * list of download bases parsed from releaseUrls (github first, gitee/others
 * after — the author usually mirrors releases). Falls back to a single
 * GitHub base with tag "v" + version.
 */
/** Bounded fetch retry for flaky mirrors (connection-level errors only). */
...
/**
 * Candidate raw URLs for the version file, mirror-aware. `knownMirrorRepos`
 * are mirror copies learned from a previous successful read (persisted in
 * provenance) — a mirror may live under a DIFFERENT owner than the GitHub
 * repo (e.g. gitee.com/qicuo/tonghuasun-agent), so same-owner fallbacks
 * alone are not enough once the primary host is down.
 */
function releaseVersionFileUrls(repo, versionFile, knownMirrorRepos = []) {
  const urls = [
    `https://raw.githubusercontent.com/${repo}/HEAD/${versionFile}`,
  ]
  for (const mirror of knownMirrorRepos) {
    urls.push(`https://gitee.com/${mirror}/raw/main/${versionFile}`)
    urls.push(`https://gitee.com/${mirror}/raw/master/${versionFile}`)
  }
  urls.push(`https://gitee.com/${repo}/raw/main/${versionFile}`)
  urls.push(`https://gitee.com/${repo}/raw/master/${versionFile}`)
  return urls
}

async function fetchReleaseVersionFile(repo, versionFile, knownMirrorRepos = []) {
  let lastError = null
  for (const url of releaseVersionFileUrls(repo, versionFile, knownMirrorRepos)) {
    try {
      const res = await fetchWithRetry(url, { headers: { 'User-Agent': 'dsh-extension-manager' }, signal: AbortSignal.timeout(12000) })
  ...
  throw lastError || new Error('版本文件读取失败')
}

/** Persist releaseUrls mirrors into provenance so later checks can reach
 * them even when the primary host is down. Returns the mirror repo list. */
function rememberReleaseMirrors(dir, meta) {
  if (!dir || !meta || !meta.releaseUrls || typeof meta.releaseUrls !== 'object') return []
  const mirrors = []
  for (const key of ['gitee', 'github']) {
    const u = meta.releaseUrls[key]
    if (typeof u !== 'string') continue
    const m = /^https?:\/\/[^/]+\/([^/]+\/[^/]+)\/releases\/tag\//.exec(u)
    if (m && m[1] !== mirrors[mirrors.length - 1]) mirrors.push(m[1])
  }
  if (mirrors.length === 0) return []
  const originPath = path.join(dir, '.dsh-plugin-origin.json')
  try {
    const v = JSON.parse(fs.readFileSync(originPath, 'utf8'))
    if (v && v.release && typeof v.release === 'object') {
      const known = Array.isArray(v.release.mirrors) ? v.release.mirrors : []
      const merged = [...new Set([...known, ...mirrors])]
      if (merged.join() !== known.join()) {
        v.release.mirrors = merged
        fs.writeFileSync(originPath, JSON.stringify(v, null, 2) + '\n', 'utf8')
      }
    }
  } catch {
    // provenance unreadable — nothing to remember into
  }
  return mirrors
}

async function resolveReleaseInfo(repo, versionFile, knownMirrorRepos = []) {
  const meta = await fetchReleaseVersionFile(repo, versionFile, knownMirrorRepos)
  const version = typeof meta.latestVersion === 'string'
    ? meta.latestVersion
    : typeof meta.version === 'string' ? meta.version : ''
  if (!version) throw new Error('版本文件缺少 latestVersion')
  const bases = []
  if (meta.releaseUrls && typeof meta.releaseUrls === 'object') {
    for (const key of ['github', 'gitee']) {
      const u = meta.releaseUrls[key]
      if (typeof u !== 'string') continue
      const m = /^(https?:\/\/[^/]+\/[^/]+\/[^/]+)\/releases\/tag\/([^/?#]+)$/.exec(u)
      if (m) bases.push({ host: key, base: `${m[1]}/releases/download`, tag: m[2] })
    }
  }
  if (bases.length === 0) bases.push({ host: 'github', base: `https://github.com/${repo}/releases/download`, tag: 'v' + version })
  return { version, bases, meta }
}

/** Expand an assetPattern (literal `<version>` placeholder) for one version. */
function releaseAssetName(pattern, version) {
  return String(pattern).replace(/<version>/g, version)
}

/** Candidate download URLs for a release asset, in mirror order. */
function releaseDownloadUrls(info, assetName) {
  return info.bases.map((b) => ({
    host: b.host,
    url: `${b.base}/${encodeURIComponent(b.tag)}/${encodeURIComponent(assetName)}`,
  }))
}

/**
 * Download a release asset trying every mirror in order; returns the first
 * successful response (already consumed? no — returns { res, host, url }).
 */
async function fetchReleaseAsset(info, assetName, timeoutMs = 120000) {
  let lastError = null
  for (const { host, url } of releaseDownloadUrls(info, assetName)) {
    try {
      const res = await fetchWithRetry(url, { signal: AbortSignal.timeout(timeoutMs) })
      if (res.ok) return { res, host, url }
      lastError = new Error(`HTTP ${res.status}`)
    } catch (error) {
      lastError = error
    }
...
/**
 * Extract a tgz (npm pack or GitHub tarball) and return the root dir that
 * holds package.json. npm pack archives carry a single `package/` root;
 * GitHub tarballs a single `owner-repo-sha/` root.
 */
async function extractTgzRoot(tgzPath, tmpDir) {
  await execFileAsync(process.platform === 'win32' ? 'tar.exe' : 'tar', ['-xzf', tgzPath, '-C', tmpDir], { timeout: 120000, windowsHide: true })
  const dirs = fs.readdirSync(tmpDir).filter((d) => d !== 'src.tgz' && fs.statSync(path.join(tmpDir, d)).isDirectory())
  if (dirs.length === 0) throw new Error('压缩包内没有可用的目录')
  const packageDir = dirs.includes('package') ? 'package' : dirs[0]
  return path.join(tmpDir, packageDir)
}

/**
 * Locate the harness's own node_modules (where official packages like
 * @deepseek-ai/dsh-mcp-client live). dsh web may run under the SYSTEM node
 * (process.execPath = C:\Program Files\nodejs\node.exe) while the harness
 * modules sit under the runtime dir, so execPath alone is NOT a reliable
 * anchor. Candidate order:
 *   1. explicit source hint (persisted provenance linkSource);
 *   2. %LOCALAPPDATA%\DeepSeek Harness\runtime\node_modules (Windows);
 *   3. ~/Library/Application Support/DeepSeek Harness/runtime/node_modules
 *      and ~/.local/share/DeepSeek Harness/runtime/node_modules;
 *   4. dirname(process.execPath)/node_modules.
 * First existing node_modules wins.
 */
function runtimeNodeModules(hint) {
  const candidates = []
  if (typeof hint === 'string' && hint !== '') candidates.push(hint)
  const localAppData = process.env.LOCALAPPDATA
  if (localAppData && localAppData.trim() !== '') {
    candidates.push(path.join(localAppData, 'DeepSeek Harness', 'runtime', 'node_modules'))
  }
  const home = os.homedir()
  candidates.push(path.join(home, 'Library', 'Application Support', 'DeepSeek Harness', 'runtime', 'node_modules'))
  candidates.push(path.join(home, '.local', 'share', 'DeepSeek Harness', 'runtime', 'node_modules'))
  try {
...
/**
 * Link a plugin's peerDependencies into its own node_modules so bare imports
 * resolve when the plugin lives outside the profile/runtime package trees
 * (junction first, plain copy fallback). `sourceHint` may carry a previously
 * persisted linkSource. Returns { ok, linked, skipped, source }.
 */
export function linkPeerDeps(target, pkg, sourceHint) {
  const peers = pkg && pkg.peerDependencies && typeof pkg.peerDependencies === 'object'
    ? Object.keys(pkg.peerDependencies).filter((k) => pkg.peerDependencies[k])
    : []
  if (peers.length === 0) return { ok: true, linked: [], skipped: [], source: null }
  const nm = runtimeNodeModules(sourceHint)
  if (!nm) return { ok: false, message: '找不到宿主 node_modules，无法链接对等依赖', linked: [], skipped: peers, source: null }
  const linked = []
  const skipped = []
  for (const name of peers) {
    const src = path.join(nm, ...name.split('/'))
    if (!fs.existsSync(src)) { skipped.push(name); continue }
    const linkPath = path.join(target, 'node_modules', ...name.split('/'))
    try {
      ...
      try {
        ...
        linked.push(name + '(copy)')
      } catch {
        skipped.push(name)
      }
    }
  }
  return { ok: skipped.length === 0, linked, skipped, source: nm }
}

/** Persist the resolved linkSource into provenance so later updates reuse it
 * even when the process cannot rediscover the harness location. */
function rememberLinkSource(dir, source) {
  if (!dir || typeof source !== 'string' || source === '') return
  const originPath = path.join(dir, '.dsh-plugin-origin.json')
  try {
    const v = JSON.parse(fs.readFileSync(originPath, 'utf8'))
    if (v && v.release && typeof v.release === 'object' && v.release.linkSource !== source) {
      v.release.linkSource = source
      fs.writeFileSync(originPath, JSON.stringify(v, null, 2) + '\n', 'utf8')
    }
  } catch {
    // provenance unreadable — nothing to remember into
...
/**
 * npm-channel installs bypass npm's dependency resolution, so runtime deps
 * declared by the plugin must be fetched explicitly. One level deep (deps of
 * deps remain the profile's pnpm concern). Returns the installed
 * name@version list and the deps that could NOT be auto-installed.
 * `profileDeps` (when provided) receives `dep: ^version` entries so a future
 * pnpm install keeps them; the CALLER persists the manifest.
 */
export async function resolveRuntimeDeps(webProfileDir, depsObj, profileDeps) {
  const installed = []
  const failed = []
  const names = depsObj && typeof depsObj === 'object'
    ? Object.keys(depsObj).filter((k) => depsObj[k])
    : []
  for (const dep of names) {
    try {
      const probe = await fetch(`https://registry.npmjs.org/${registryName(dep)}/latest`, { signal: AbortSignal.timeout(10000) })
      if (!probe.ok) { failed.push(dep); continue }
      const meta = await probe.json()
      const r = await updateNpmPackage(webProfileDir, dep)
      if (r.ok) {
        installed.push(`${dep}@${r.version || meta.version || '?'}`)
        if (profileDeps && typeof profileDeps[dep] !== 'string') {
          profileDeps[dep] = '^' + (meta.version || r.version || '0.0.0')
        }
      } else {
        failed.push(dep)
      }
...
export async function checkPluginUpdates(ctx, webProfileDir) {
  ...
  for (const plugin of snapshot) {
    ...
    if (path.isAbsolute(root)) {
      ...
      // GitHub Release channel: provenance carries a release assetPattern —
      // compare against the author's version file instead of the repo root
      // package.json (which may not exist for release-distributed plugins).
      const originFull = readPluginOriginFull(root)
      if (originFull && originFull.release) {
        try {
          const info = await resolveReleaseInfo(repo, originFull.release.versionFile, originFull.release.mirrors || [])
          rememberReleaseMirrors(root, info.meta)
          results.push({
            name,
            kind: 'release',
            repo,
            ...
            latest: info.version,
            updateable: !!info.version && compareVersions(info.version, localVersion) > 0,
          })
        } catch {
          results.push({ name, kind: 'release', repo, current: localVersion, latest: '', updateable: false, error: true })
        }
        continue
      }
      try {
 ...
 *   GitHub Release dir       → same swap against the author's release asset
 *                              (releaseUpdate);
...
export async function updatePluginItem(webDir, name) {
  ...
  if (path.isAbsolute(root)) {
    ...
    const originFull = readPluginOriginFull(root)
    if (originFull && originFull.release) {
      const r = await releaseUpdate(root, repo, originFull.release)
      return { ...r, kind: 'release' }
    }
...
/**
 * Auto-update a GitHub Release plugin dir from the author's release asset:
 * read the version file, download the `<name>-<version>.tgz`, validate it
 * like a fresh install would, then swap directories with backup + rollback.
 * Refusing is deliberate when the new asset fails validation or its embedded
 * version disagrees with the version file.
 */
async function releaseUpdate(root, repo, release) {
  const localPkg = readJson(path.join(root, 'package.json'))
  const localVersion = localPkg && typeof localPkg.version === 'string' ? localPkg.version : ''
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-relupd-'))
  try {
    const info = await resolveReleaseInfo(repo, release.versionFile, release.mirrors || [])
    if (!info.version || compareVersions(info.version, localVersion) <= 0) {
      return { ok: true, message: `已是最新（v${localVersion || '?'}）` }
    }
    const assetName = releaseAssetName(release.assetPattern, info.version)
    const tgzPath = path.join(tmpDir, 'pkg.tgz')
    let fetched
    try {
      fetched = await fetchReleaseAsset(info, assetName)
    } catch (error) {
      return { ok: false, message: `下载发行包失败（${assetName}）：${error && error.message ? error.message : String(error)}` }
    }
    fs.writeFileSync(tgzPath, Buffer.from(await fetched.res.arrayBuffer()))
    const srcBase = await extractTgzRoot(tgzPath, tmpDir)
    const verdict = validateClonePackage(srcBase)
    if (!verdict.ok) {
      return {
        ok: false,
        code: 'invalid',
        message: '远端发行包未通过安装校验（' + verdict.message + '）。为避免静默装上无法加载的产物，本次未更新',
      }
    }
    const pkgVersion = verdict.pkg && typeof verdict.pkg.version === 'string' ? verdict.pkg.version.replace(/^v/, '') : ''
    if (pkgVersion !== info.version) {
      return { ok: false, code: 'invalid', message: `发行包版本不符（包内 ${pkgVersion || '?'} ≠ 版本文件 ${info.version}），本次未更新` }
    }
    const backup = root + '.bak'
    try { fs.rmSync(backup, { recursive: true, force: true }) } catch { /* keep going */ }
    fs.renameSync(root, backup)
    try {
      fs.cpSync(srcBase, root, { recursive: true })
      writePluginOrigin(root, repo, release)
      rememberReleaseMirrors(root, info.meta)
      const link = linkPeerDeps(root, verdict.pkg, release.linkSource)
      rememberLinkSource(root, link.source)
      try { fs.rmSync(backup, { recursive: true, force: true }) } catch { /* keep going */ }
      let message = `已更新 ${repo} 至 v${pkgVersion}（原 v${String(localVersion).replace(/^v/, '') || '?'}，下载源：${fetched.host}），重启 dsh web 后生效`
      if (link.linked && link.linked.length > 0) message += '；已链接对等依赖：' + link.linked.join(', ')
      if (link.skipped && link.skipped.length > 0) message += '；⚠ 对等依赖未链接：' + link.skipped.join(', ')
