...
function registerCloneRow(webProfileDir, slug, base, pkg) {
  ...
  if (!wrote) {
    return { ok: false, message: '注册行未能写入配置，请重启 dsh web 后重试' }
  }
  return { ok: true }
}
...
export async function installPlugin(webProfileDir, repo, subdir) {
  ...
  if (fs.existsSync(target)) {
    ...
    if (verdict.ok) {
      ...
      if (verdict.pkg.dsh && verdict.pkg.dsh.bundle && typeof verdict.pkg.dsh.bundle.patch === 'string') {
 ...
 * origin; ZIP/manual installs (no .git) compare package.json versions against
 * the repo recorded in their `.dsh-plugin-origin.json` — report-only, since
 * auto-replacing source without rebuilding dist would serve stale artifacts.
...
export async function checkPluginUpdates(ctx, webProfileDir) {
  ...
  for (const plugin of snapshot) {
    ...
    if (path.isAbsolute(root)) {
      ...
      // No .git: provenance-file based version check (report-only).
      const repo = readPluginOrigin(root)
      if (!repo) {
