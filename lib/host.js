...
import { upsertMcp, removeMcp, getMcpEntry } from './mcp.mjs'
import { dshHome } from './paths.mjs'
import { readState, writeState } from './state.mjs'
import { normalizeServers, withServers, SERVER_MODES, assertMode } from './server-mode.mjs'
  ...
  async setServerMode(input) {
    ...
    if (!config && id !== '') {
      ...
      if (entry && entry.config) {
        config = entry.config
        serverName = entry.config.serverName || id.replace(/^mcp-/, '')
      }
    }
    ...
    if (id !== '') setServerDisabled(this._patchPath(), id, mode !== 'off')
    return this._applySingle(serverName)
  }
