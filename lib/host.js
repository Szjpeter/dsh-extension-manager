...
import { listPlugins as pluginSnapshot, removePluginRow, checkPluginUpdates as pluginUpdateScan, updatePluginItem, installReleasePlugin as installReleasePluginFn } from './plugins.mjs'
...
class extensionManagerGateway extends TypertRemoteService {
  ...
  _assertSkillWritable(name, scope, folder) {
    ...
    if (hit && hit.readOnly) {
      throw new Error(`技能"${name}" 来自只读域（${hit.source}），不能直接修改。请在用户目录新建同名技能。`)
    }
  }
  ...
  // Install a plugin from a GitHub Release tgz (author-distributed build
  // artifact). `release` carries { assetPattern, versionFile? }; defaults to
  // `<repoBase>-<version>.tgz` + update/stable.json.
  async installReleasePlugin(input) {
    input = input || {}
    const repo = typeof input.repo === 'string' ? input.repo.trim() : ''
    if (!repo || !repo.includes('/')) throw new Error('missing repo (owner/name)')
    const release = input.release && typeof input.release === 'object' ? input.release : undefined
    return jsonSafe(await installReleasePluginFn(profileDir(), repo, release))
  }
  ...
  _qualityGate(entryId, { requireResolvable = true } = {}) {
    ...
    if (!row) {
      ...
      problems.push(`在可见组合层中未找到行「${entryId}」`)
...
for (const [method, exportName] of [
  ...
  ['installReleasePlugin', undefined],
  ['getState', undefined],
  ['setState', undefined],
