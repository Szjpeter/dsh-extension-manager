...
import { removeSkill, toggleSkill } from './install.mjs'
...
import { listUserRepos, detectRepoUnits, detectRepoUnitsWithLister, installSkillFromRepo, fetchRawFile, setRepoNameHint } from './github.mjs'
import { createGitHubBridge } from './github-bridge.mjs'
import { kebab } from './skills.mjs'
import fs from 'node:fs'
import { installPlugin as gitInstallPlugin } from './plugins.mjs'
...
class extensionManagerGateway extends TypertRemoteService {
  constructor(ctx) {
    ...
    runRemoteMarks(this)
    // Lazy-bridge runtime state (never serialized).
    this._lazyClients = new Map()   // serverName -> McpLite instance
    this._lazyDisposers = new Map() // serverName -> [disposer]
    this._lazyErrors = new Map()    // serverName -> error string
    this._lazyTouched = false
  }

  // Activate stub registrations for every configured lazy server. Fire-and-
  // forget: failures land in _lazyErrors and surface through getLazy(), never
  // through the caller's stack.
  _ensureLazyBridges() {
    if (this._lazyTouched) return
    ...
    try {
      ...
      const servers = readState().lazyServers || []
      for (const cfg of servers) {
        if (!cfg || !cfg.serverName) continue
        if (this._lazyClients.has(cfg.serverName)) continue
        this._activateBridge(tools, cfg).catch((e) => {
          this._lazyErrors.set(cfg.serverName, e && e.message ? e.message : String(e))
        })
      }
    } catch (error) {
      // containment: a bridge setup failure must not break list/status calls
      this._lazySetupError = error && error.message ? error.message : String(error)
    }
  }

  async _activateBridge(tools, cfg) {
    const client =
      cfg.transport === 'stdio' ? new McpLiteStdio(cfg) : new McpLiteHttp(cfg)
    ...
    for (const t of realTools) {
      ...
      const stubName = `mcp__${cfg.serverName}__${t.name}`
      try {
        const dispose = tools.register({
          name: stubName,
          description: `${t.description || t.name}（懒加载：参数为自由 JSON 对象）`,
          parameters: {
            arguments: {
              type: 'object',
              required: false,
          ...
          execute: async (args) => {
            const live =
              this._lazyClients.get(cfg.serverName) ||
              (await this._activateBridge(tools, cfg).then(() => this._lazyClients.get(cfg.serverName)))
            if (!live) throw new Error(`懒加载桥未连接：${cfg.serverName}`)
            const res = await live.request('tools/call', {
              name: t.name,
              arguments: (args && args.arguments) || {},
            })
            return jsonSafe(res && typeof res === 'object' ? res : { value: res })
          },
        })
        if (typeof dispose === 'function') disposers.push(dispose)
      } catch (e) {
        // one bad stub must not sink the rest of the server's tools
        this._lazyErrors.set(`${cfg.serverName}/${t.name}`, e && e.message ? e.message : String(e))
      }
    }
    this._lazyClients.set(cfg.serverName, client)
    this._lazyDisposers.set(cfg.serverName, disposers)
    this._lazyErrors.delete(cfg.serverName)
  }

  // ── lazy-loading configuration ─────────────────────────────────────────────

  getLazy() {
    this._ensureLazyBridges()
    return jsonSafe({
      servers: readState().lazyServers || [],
      active: [...this._lazyClients.keys()],
      errors: Object.fromEntries(this._lazyErrors),
      setupError: this._lazySetupError || null,
    })
  }

  // Enable lazy loading for an existing MCP row: copy its config into the
  // persistent lazy list, register stubs immediately, and toggle the native
  // dsh-mcp-client row off (effective next restart; runtime is already served
  // by the bridge).
  async enableLazy(input) {
    input = input || {}
    const id = typeof input.id === 'string' ? input.id.trim() : ''
    if (id === '') throw new Error('missing mcp row id')
    if (input.confirm !== true) throw new Error('开启懒加载需要显式确认（stub 不携带完整参数 schema）')
    const entry = getMcpEntry(id, 'global', process.cwd())
    if (!entry || !entry.config) throw new Error(`未找到 MCP 行：${id}`)
    const serverName = entry.config.serverName || id.replace(/^mcp-/, '')
    const servers = readState().lazyServers || []
    if (!servers.some((s) => s.serverName === serverName)) {
      servers.push(Object.assign({ serverName }, entry.config))
      writeState({ lazyServers: servers })
    }
    setServerDisabled(this._patchPath(), id, true)
    this._lazyTouched = false
    this._ensureLazyBridges()
    return jsonSafe({
      ok: true,
      serverName,
      note: '桥已即时注册；原生行已标记停用（重启后不再加载其全量 schema）',
    })
  }
  ...
  disableLazy(input) {
    ...
    const servers = (readState().lazyServers || []).filter((s) => s.serverName !== serverName)
    writeState({ lazyServers: servers })
    for (const d of this._lazyDisposers.get(serverName) || []) {
      try {
        d()
      } catch {
        // already disposed
    ...
    this._lazyDisposers.delete(serverName)
    const client = this._lazyClients.get(serverName)
    if (client) {
      Promise.resolve(client.close()).catch(() => {})
      this._lazyClients.delete(serverName)
    }
    this._lazyErrors.delete(serverName)
    // Re-arm so a later enable re-registers within this session.
    this._lazyTouched = false
  ...
  getMcp(input) {
    input = input || {}
    return jsonSafe(getMcpEntry(input.id, input.scope || 'global', process.cwd()))
  }
  ...
  removeMcp(input) {
    input = input || {}
    return removeMcp(input.id || input.name, input.scope || 'global', process.cwd())
  }

  toggleMcp(input) {
    input = input || {}
    return toggleMcp(input.id || input.name, !!input.disabled, input.scope || 'global', process.cwd())
  }
  ...
  async gitInstallSkill(input) {
    ...
    try {
      ...
      if (text !== null) {
        // write via the same installer body but with content already in hand
        const guess = String(input.path || '').split('/').filter(Boolean)
        const guessed = guess.length >= 2 ? guess[guess.length - 2] : null
        const name = kebab(input.name || guessed || '')
        if (!name) throw new Error('无法从路径推导技能名')
        const dirPath = path.join(dshHome(), 'skills', name)
        const file = path.join(dirPath, 'SKILL.md')
        const exists = fs.existsSync(file)
        if (exists && input.force !== true) return jsonSafe({ ok: false, exists: true, name, file })
        fs.mkdirSync(dirPath, { recursive: true })
        fs.writeFileSync(file + '.tmp', text, 'utf8')
        fs.renameSync(file + '.tmp', file)
        return jsonSafe({ ok: true, name, file, overwritten: exists })
      }
  ...
  removePlugin(input) {
    ...
    if (rawId === '') throw new Error('missing plugin id')
    const id = bareEntryId(rawId)
    const gate = this._qualityGate(id, { requireResolvable: true })
