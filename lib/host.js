...
// dependencies and bundle entries. Deliberately NOT the in-memory loader —
// after removePlugin the loader still carries the row until restart, and the
...
class extensionManagerGateway extends TypertRemoteService {
  ...
  // ── residual cleanup (uninstall leftovers) ────────────────────────────────
  ...
  // plugin — the "health check" for plugins uninstalled before this feature
  ...
  purgePlugin(input) {
    ...
    for (const n of names) {
      if (LOCKED_PLUGIN_NAMES.has(n) || n === 'dsh-extension-manager' || n === SELF_ENTRY_ID) {
        return jsonSafe({ ok: false, message: `Plugin「${n}」is protected, cleanup is forbidden` })
      }
      if (!input.force && live.has(n.toLowerCase())) {
        return jsonSafe({ ok: false, message: `Plugin「${n}」is still registered in the composition, please uninstall it first (or restart dsh web to make the uninstall take effect) before cleanup` })
      }
    }
