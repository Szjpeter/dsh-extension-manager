	...
	factory: (require) => {
		...
		function McpTab(props) {
			...
			if (!it.readOnly) {
				...
				// M5: probe works for BOTH transports (probeHttp handles
				// streamable-http) and for every non-read-only layer — the old
				// stdio-only gate hid half the rows and probeMcpById only saw the
				// global patch anyway.
				if (!it.readOnly) {
					actions.push(
