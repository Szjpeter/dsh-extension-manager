...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			"mcp.mode.off": "停用",
			"mcp.mode.current": "当前",
			"mcp.mode.confirm.full": "标准模式会立即建立真实连接，并以服务器返回的完整参数 schema 注册全部工具。确认？",
			"mcp.mode.confirm.lazy": "懒加载模式仅注册名称级 stub（无完整参数 schema），可能影响模型对参数的理解精度。确认？",
		...
		var en = {
			...
			"mcp.mode.confirm.full": "Standard mode opens a real connection now and registers every tool with the server-provided parameter schema. Continue?",
			"mcp.mode.confirm.lazy": "Lazy mode registers name-only stubs (no full parameter schema); model accuracy on arguments may degrade. Continue?",
		...
		function McpTab(props) {
			...
			// v0.2 gateway control plane: full / lazy / off — applied immediately
			// by the host bridge (register/dispose in-process), no restart.
			function doSetMode(item, mode) {
				var confirmKey = { full: "mcp.mode.confirm.full", lazy: "mcp.mode.confirm.lazy" }[mode];
				if (mode !== "off" && confirmKey && !window.confirm(t(confirmKey))) return;
				withBusy(function () {
			...
			function doToggle(item) {
				withBusy(function () {
					// v0.2 fix: pass the explicit target — item.enabled is the
					// CURRENT state, so disabled = !enabled. The 0.1 client
					// omitted this field and the host defaulted it to false,
					// making every toggle a silent no-op in one direction.
					return call("toggleMcp", { id: item.id, disabled: !!item.enabled }).then(function () {
						setMsg({ kind: "ok", text: t("msg.pendingRestart") });
					});
				});
			}
			...
			var fidelityLabel = gw === "lazy" ? t("mcp.fidelity.econ") : t("mcp.fidelity.standard");
			...
			if (!it.readOnly) {
				if (gw) {
					// The ONE switch. Turning on reconnects as 标准 (full) by default;
					// turning off keeps the row gateway-managed but silent.
					actions.push(Btn({
						key: "switch",
						kind: swOn ? "primary" : undefined,
						onClick: function () { if (!busy) doSetMode(it, swOn ? "off" : "full"); },
						disabled: busy,
					}, swOn ? t("mcp.sw.off") : t("mcp.sw.on")));
					if (swOn) {
						actions.push(Btn({
							key: "fidelity",
							...
							disabled: busy,
						}, t("mcp.fidelity.label") + ": " + fidelityLabel));
					}
				} else {
					actions.push(Btn({ onClick: function () { doToggle(it); }, disabled: busy }, uiOn ? t("mcp.sw.off") : t("mcp.sw.on")));
				}
				if (isStdio && !it.readOnly) {
					actions.push(
