...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			"plugins.scanOrphans": "扫描残留",
			"plugins.scanningOrphans": "正在扫描残留…",
			"plugins.noOrphans": "未发现卸载残留",
			"plugins.leftoverConfirmPrefix": "卸载成功，检测到以下残留：",
			"plugins.leftoverConfirmSuffix": "是否一并删除？（数据目录 + node_modules 副本 + package.json 依赖/bundle 登记）",
			"plugins.leftoverDone": "残留清理完成，释放 {mb} MB",
			"plugins.leftoverFailed": "部分残留未能清理（文件可能被占用）：{list}",
			"plugins.leftoverRefused": "残留清理被安全校验拒绝：{list}",
			"plugins.orphanConfirmPrefix": "发现 {n} 项卸载残留，共约 {mb} MB：",
			"plugins.orphanConfirmSuffix": "确认删除这些残留目录？",
			"git.user": "GitHub 用户名",
			"git.load": "拉取仓库",
			"git.browse": "浏览内容",
			"git.refresh": "刷新",
		...
		var en = {
			...
			"plugins.scanOrphans": "Scan leftovers",
			"plugins.scanningOrphans": "Scanning leftovers…",
			"plugins.noOrphans": "No uninstall leftovers found",
			"plugins.leftoverConfirmPrefix": "Uninstalled. Leftover artifacts detected:",
			"plugins.leftoverConfirmSuffix": "Delete them as well? (data dirs + node_modules copy + package.json dep/bundle entry)",
			"plugins.leftoverDone": "Leftovers cleaned, {mb} MB freed",
			"plugins.leftoverFailed": "Some leftovers could not be removed (files may be locked): {list}",
			"plugins.leftoverRefused": "Some leftover paths were refused by safety checks: {list}",
			"plugins.orphanConfirmPrefix": "{n} leftover artifact(s) found, ~{mb} MB total:",
			"plugins.orphanConfirmSuffix": "Delete these leftover directories?",
		...
		// ── remote contribution ─────────────────────────────────────────────────
		var REMOTE_METHODS = ["ping", "list", "getSkill", "createSkill", "updateSkill", "removeSkill", "toggleSkill", "getMcp", "upsertMcp", "removeMcp", "toggleMcp", "probeMcp", "checkMcpUpdate", "listPlugins", "setPluginEnabled", "removePlugin", "pluginLeftovers", "scanOrphanLeftovers", "purgePlugin", "checkPluginUpdates", "updateOnePlugin", "precheckPlugin", "restoreRemovedPlugin", "mcpStatus", "getHotReload", "setHotReload", "getLazy", "enableLazy", "disableLazy", "setServerMode", "gitRepos", "gitBrowse", "gitInstallSkill", "gitInstallPlugin", "installReleasePlugin", "getState", "setState"];
		...
		function PluginsTab(props) {
			...
			function runRemove(item, force) {
				...
				call("removePlugin", force ? { id: item.entryId, force: true } : { id: item.entryId })
					.then(function (r) {
						...
						reload();
						if (r && r.ok && r.leftovers) offerLeftoverCleanup(item, r.leftovers);
					})
					.catch(function (e) {
			...
			function formatPurgeProblems(r) {
				var parts = [];
				var refused = (r && r.refused) || [];
				for (var i = 0; i < refused.length; i++) parts.push(refused[i].path + " (" + (refused[i].reason || "refused") + ")");
				var failed = (r && r.failed) || [];
				for (var j = 0; j < failed.length; j++) parts.push(failed[j].path + " (" + (failed[j].reason || "failed") + ")");
				return parts.join("\n");
			}
			function doPurge(item, lo) {
				setBusy(true);
				setMsg(null);
				call("purgePlugin", {
					id: item.entryId,
					name: item.name || "",
					dirs: (lo.dirs || []).map(function (d) { return d.path; }),
					removePackage: !!(lo.packageJson && (lo.packageJson.dep || lo.packageJson.bundle)),
					removeNodeModules: !!(lo.nodeModules && lo.nodeModules.exists),
				}).then(function (r) {
					if (r && r.ok) {
						setMsg({ kind: "ok", text: t("plugins.leftoverDone", { mb: ((r.freed || 0) / 1048576).toFixed(2) }) });
					} else {
						var refused = r && r.refused && r.refused.length;
						setMsg({ kind: "err", text: t(refused ? "plugins.leftoverRefused" : "plugins.leftoverFailed", { list: formatPurgeProblems(r) }) });
					}
				}).catch(function (e) { setMsg({ kind: "err", text: errText(e) }); })
					.finally(function () { setBusy(false); });
			}
			function offerLeftoverCleanup(item, lo) {
				if (!lo || !lo.count) return;
				var lines = [];
				var dirs = lo.dirs || [];
				for (var i = 0; i < dirs.length; i++) lines.push(dirs[i].path);
				if (lo.packageJson && (lo.packageJson.dep || lo.packageJson.bundle)) lines.push("package.json: " + (item.name || lo.name || ""));
				if (lo.nodeModules && lo.nodeModules.exists) lines.push(lo.nodeModules.path);
				if (!lines.length) return;
				var text = t("plugins.leftoverConfirmPrefix") + "\n\n" + lines.join("\n") + "\n\n" + t("plugins.leftoverConfirmSuffix");
				if (window.confirm(text)) doPurge(item, lo);
			}
			function doScanOrphans() {
				setBusy(true);
				setMsg(null);
				call("scanOrphanLeftovers", {}).then(function (r) {
					var orphans = (r && r.orphans) || [];
					if (!orphans.length) { setMsg({ kind: "ok", text: t("plugins.noOrphans") }); return null; }
					var lines = orphans.map(function (o) { return o.path + " (" + (o.bytes / 1048576).toFixed(1) + " MB)"; });
					var text = t("plugins.orphanConfirmPrefix", { n: String(orphans.length), mb: ((r.totalBytes || 0) / 1048576).toFixed(1) }) + "\n\n" + lines.join("\n") + "\n\n" + t("plugins.orphanConfirmSuffix");
					if (!window.confirm(text)) return null;
					return call("purgePlugin", { dirs: orphans.map(function (o) { return o.path; }) }).then(function (r2) {
						if (r2 && r2.ok) setMsg({ kind: "ok", text: t("plugins.leftoverDone", { mb: ((r2.freed || 0) / 1048576).toFixed(2) }) });
						else {
							var refused = r2 && r2.refused && r2.refused.length;
							setMsg({ kind: "err", text: t(refused ? "plugins.leftoverRefused" : "plugins.leftoverFailed", { list: formatPurgeProblems(r2) }) });
						}
					});
				}).catch(function (e) { setMsg({ kind: "err", text: errText(e) }); })
					.finally(function () { setBusy(false); });
				...
				return h("div", { key: item.entryId, className: "exm-row" },
					h("div", { className: "exm-main" },
						h("div", { className: "exm-name-line" },
							...
							h("span", { className: "exm-name" }, item.displayName || item.name || item.entryId),
							tierBadge(t, item),
			...
			return h("div", { className: "exm-wrap" },
				...
				h("div", { className: "exm-toolbar" },
					...
					subTab === "other"
						...
						: null,
					subTab === "other"
						? Btn({ onClick: function () { doScanOrphans(); }, disabled: busy }, t("plugins.scanOrphans"))
						: null,
					(function () {
