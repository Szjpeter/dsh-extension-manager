...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			"plugins.updateManualBadge": "可更新·需手动",
		...
		var en = {
			...
			"plugins.updateManualBadge": "update · manual",
		...
		function PluginsTab(props) {
			...
			// ZIP/manual installs: updateable but requires a manual re-download +
			// rebuild, so they get a badge instead of the one-click update button.
			var manualState = React.useState({});
			var manualUpdatable = manualState[0];
			var setManualUpdatable = manualState[1];
			...
			function doCheckUpdates() {
				...
				call("checkPluginUpdates", {}).then(function (r) {
					// Field contract: core returns `updateable` (git/npm → auto
					// updatable; zip → updateable but MANUAL rebuild required).
					...
					if (candidates) {
						for (var i = 0; i < candidates.length; i++) {
							...
							if (!nm || !flag) continue;
							if (it.kind === "zip") manual[nm] = true;
							else found[nm] = true;
						}
					}
					setUpdatable(found);
					setManualUpdatable(manual);
					var names = Object.keys(found).concat(Object.keys(manual).map(function (n) { return n + "*"; }));
					setMsg({ kind: "ok", text: names.length ? t("plugins.updatableFound") + ": " + names.join(", ") : t("plugins.allUpToDate") });
				}).catch(function (e) { setMsg({ kind: "err", text: errText(e) }); })
					.finally(function () { setBusy(false); });
			}
			...
			function pluginRow(item) {
				...
				// ZIP/manual installs: show the flag, no one-click update (source
				// replacement without a rebuild would serve stale artifacts).
				var manualUpdate = !item.official && !!manualUpdatable[item.name];
				return h("div", { key: item.entryId, className: "exm-row" },
					h("div", { className: "exm-main" },
						h("div", { className: "exm-name-line" },
							...
							manualUpdate ? h(Badge, { text: t("plugins.updateManualBadge"), state: "on" }) : null,
