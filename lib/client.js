	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".exm-tab{cursor:pointer;border:none;background:none;font-family:inherit;font-size:14px;color:var(--dsw-alias-label-secondary,#646a73);padding:6px 14px;border-radius:10px}",
				...
				".exm-spacer{flex:1}",
				".exm-btn{cursor:pointer;border:1px solid var(--dsw-alias-border,#d0d3d9);background:var(--dsw-alias-bg-layer-2,#fff);color:var(--dsw-alias-label-primary,#1f2329);font-family:inherit;font-size:13px;padding:5px 12px;border-radius:9px}",
				...
				".exm-item{display:flex;align-items:center;gap:10px;padding:8px 10px;border:1px solid var(--dsw-alias-border,#e5e6eb);border-radius:10px;background:var(--dsw-alias-bg-layer-2,#fff)}",
				...
				".exm-badge{font-size:11px;padding:1px 8px;border-radius:999px;border:1px solid var(--dsw-alias-border,#d0d3d9);color:var(--dsw-alias-label-secondary,#646a73)}",
				...
				".exm-msg{font-size:13px;padding:6px 10px;border-radius:8px}",
				...
				".exm-input,.exm-select,.exm-textarea{box-sizing:border-box;width:100%;border:1px solid var(--dsw-alias-border,#d0d3d9);background:var(--dsw-alias-bg-layer-2,#fff);color:var(--dsw-alias-label-primary,#1f2329);font-family:inherit;font-size:13px;padding:5px 9px;border-radius:8px}",
				...
				".exm-seg{display:inline-flex;border:1px solid var(--dsw-alias-border,#d0d3d9);border-radius:9px;overflow:hidden}",
		...
		var zh = {
			...
			"mcp.lazyOn": "懒加载：开",
			"mcp.lazyOff": "懒加载：关",
			"mcp.lazyConfirm": "开启后该 MCP 服务器的工具将以轻量桩形式注册（仅名称+描述，无完整参数 schema），首次调用时才建连拉取真实 schema。这可能影响模型对参数的理解精度。确认？",
			"mcp.lazyNote": "仅 stdio 型服务器支持懒加载（网络型无 schema 开销）",
			"status.title": "MCP 状态",
			...
			"common.readonly": "只读",
			"common.builtin": "内置",
			"plugins.placeholder": "插件管理在 Phase D 提供：分层展示 / 启停 / 卸载 / 更新检查 / harness 运行树快照。",
			"plugins.locked": "已锁定",
			"plugins.confirm": "受保护",
			"plugins.free": "第三方",
			"plugins.official": "官方",
		...
		var en = {
			...
			"mcp.lazyOn": "lazy: on",
			"mcp.lazyOff": "lazy: off",
			"mcp.lazyConfirm": "Enables lightweight stub registration (name + description only, no full parameter schema) for this MCP server. The real connection happens on first call. This may reduce model accuracy. Continue?",
			"mcp.lazyNote": "Lazy loading only applies to stdio servers (remote servers have no local schema cost)",
			...
			"plugins.placeholder": "Plugin management arrives in Phase D: layered view / toggle / uninstall / update check / harness runtime snapshot.",
		...
		// ── remote contribution ─────────────────────────────────────────────────
		var REMOTE_METHODS = ["ping", "list", "getSkill", "createSkill", "updateSkill", "removeSkill", "toggleSkill", "getMcp", "upsertMcp", "removeMcp", "toggleMcp", "probeMcp", "checkMcpUpdate", "listPlugins", "setPluginEnabled", "removePlugin", "checkPluginUpdates", "updateOnePlugin", "precheckPlugin", "restoreRemovedPlugin", "mcpStatus", "getHotReload", "setHotReload", "getLazy", "enableLazy", "disableLazy", "gitRepos", "gitBrowse", "gitInstallSkill", "gitInstallPlugin", "getState", "setState"];
		...
		function McpTab(props) {
			...
			function doToggleLazy(item) {
				if (lazyActive.has(item.id)) {
					setBusy(true);
					setMsg(null);
					call("disableLazy", { serverName: item.serverName }).then(function () {
						setLazyActive(new Set());
						reload();
					}).catch(function (e) { setMsg({ kind: "err", text: errText(e) }); })
						.finally(function () { setBusy(false); });
					return;
				}
				if (!window.confirm(t("mcp.lazyConfirm"))) return;
				setBusy(true);
				setMsg(null);
				call("enableLazy", { id: item.id, confirm: true }).then(function () {
			...
			function doToggle(item) {
				withBusy(function () {
					return call("toggleMcp", { id: item.id }).then(function () {
						setMsg({ kind: "ok", text: t("msg.pendingRestart") });
					});
				});
			}
			function doRemove(item) {
				...
				withBusy(function () {
					return call("removeMcp", { id: item.id }).then(function () {
						setMsg({ kind: "ok", text: t("msg.pendingRestart") });
					});
				});
			}
			...
			var rows = (items || []).map(function (it) {
				...
				var isStdio = it.transport === "stdio";
				var lines = [];
				if (probe) {
				...
				return h("div", { key: it.id + "/" + it.scope, className: "exm-row" },
					...
					h("div", { className: "exm-actions" },
						it.readOnly ? null : Btn({ onClick: function () { doToggle(it); }, disabled: busy }, it.enabled ? t("mcp.disable") : t("mcp.enable")),
						isStdio && !it.readOnly
						...
						isStdio && !it.readOnly
							...
							: null,
						isStdio && !it.readOnly
							? Btn({ onClick: function () { doToggleLazy(it); }, disabled: busy }, lazyActive.has(it.id) ? t("mcp.lazyOn") : t("mcp.lazyOff"))
							: null,
						it.readOnly ? null : Btn({ className: "exm-btn exm-danger", onClick: function () { doRemove(it); }, disabled: busy }, t("mcp.remove"))));
			});
		...
		function PluginsTab(props) {
			...
			function doRemove(item) {
				...
				setBusy(true);
				setMsg(null);
				call("removePlugin", { id: item.entryId })
					.then(function (r) {
			...
			function doCheckUpdates() {
				...
				call("checkPluginUpdates", {}).then(function (r) {
					// Tolerant extraction — the scan result shape is the persistence
					// core's; find a list of {name, status|updateAvailable} anywhere.
					var found = {};
				 var candidates = null;
					...
					if (candidates) {
						for (var i = 0; i < candidates.length; i++) {
							...
							var flag = it.updateAvailable === true || it.hasUpdate === true || it.status === "update-available";
							if (nm && flag) found[nm] = true;
						}
					}
					setUpdatable(found);
					var names = Object.keys(found);
					setMsg({ kind: names.length ? "ok" : "ok", text: names.length ? t("plugins.updatableFound") + ": " + names.join(", ") : t("plugins.allUpToDate") });
				}).catch(function (e) { setMsg({ kind: "err", text: errText(e) }); })
					.finally(function () { setBusy(false); });
			}
			...
			if (selected && units) {
				...
				var pluginRows = (units.plugins || []).map(function (pl) {
					...
					var kindText = kind === "dsh-plugin" ? t("git.kind.dsh")
						: kind === "mcp-server" ? t("git.kind.mcp") : t("git.kind.unknown");
					var note = kind === "mcp-server" ? t("git.mcpHint") : null;
