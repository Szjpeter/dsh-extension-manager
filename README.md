...
- **交互梳理（MCP 行按钮矩阵，按用户裁定收敛）**：每个可管理行**只有一个开关**——
  开启=进入网关（标准模式，原生行钉扎、进程内注册工具），关闭=网关静默且保持托管；
  由此废除原生「启用/禁用」与网关开关并存的双轨（托管记录丢失后无法回到托管的
  死角随之消除）。「工具参数（完整参数/省上下文）」仅在开启后显示，单击即切、
  无弹窗；「测试 / 查更新」回归为 **仅 stdio 行** 显示；模式切换不再弹确认框
  （真正高危的热重载确认保留）。
- **Git 分类器加固**：package.json 读取失败不再被静默标为「非 DSH 插件」——
  自动重试一次后仍失败显示独立的「读取失败」徽章并提示刷新；同目录存在
  `cordis.patch.yml` / `agent.cordis.yml` 时即使没有 `dsh` 字段也识别为 DSH 插件
  （SodaMem/dsh-plugin-sodamem 即曾被误标）。
- **M5 多作用域矩阵闭合**：MCP 行的 查询/启停/删除/探测/查更新 现在按行的**实际所在层**
  （home 补丁 / profile 补丁 / 预设 / 项目 manifest）分发；预设行可正常测试与启停删除，
  只读层与"仅存于 manifest"的行给出明确业务错误而非假成功。删除不存在 id 会显示无操作提示。
- **ZIP/手动安装的更新检查**：无 `.git` 的插件目录通过目录内 `.dsh-plugin-origin.json`
  （`{"repo":"owner/repo"}`；Git 页克隆安装自动写入，AI/手动安装建议照此补一份）记录
  来源仓库；「检查更新」对这类目录改用 本地 package.json 版本 vs 远端 package.json 版本
  比较，可更新时显示「可更新·需手动」徽章——源码更新后必须重新构建，因此不做自动替换。
  无来源档案的目录显示"来源未记录"，不再被静默跳过；同时修正 client 端字段名笔误
  （`updateable` 被读成 `updateAvailable`，导致 v0.2.0 起「更新」按钮从未点亮）。
